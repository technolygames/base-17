    ...
    protected final void botones(){
        ...
        miStorePic.addActionListener((a)->{
            try{
                ...
                new logger(Level.INFO).staticLogger("Se guardó correctamente la imagen del empleado.\nOcurrió en la clase '"+dataWindow4.class.getName()+"', en el método 'botones(miStorePic)'.\nUsuario que hizo la acción: "+String.valueOf(start.userID));
                
                ps.close();
            }catch(SQLException e){
                JOptionPane.showMessageDialog(null,"Error:\n"+e.getMessage(),"Error 14",JOptionPane.ERROR_MESSAGE);
                new logger(Level.SEVERE).staticLogger("Error 14: "+e.getMessage()+".\nOcurrió en la clase '"+dataWindow4.class.getName()+"', en el método 'botones(miStorePic)'");
                new logger(Level.SEVERE).exceptionLogger(dataWindow4.class.getName(),"botones.miStorePic-14",e.fillInStackTrace());
            }catch(FileNotFoundException x){
                JOptionPane.showMessageDialog(null,"Error:\n"+x.getMessage(),"Error 1IO",JOptionPane.ERROR_MESSAGE);
                new logger(Level.SEVERE).staticLogger("Error 1IO: "+x.getMessage()+".\nOcurrió en la clase '"+dataWindow4.class.getName()+"', en el método 'botones(miStorePic)'");
                new logger(Level.SEVERE).exceptionLogger(dataWindow4.class.getName(),"botones.miStorePic-10",x.fillInStackTrace());
            }catch(NullPointerException n){
                JOptionPane.showMessageDialog(null,"Error:\n"+n.getMessage(),"Error 0",JOptionPane.ERROR_MESSAGE);
                new logger(Level.SEVERE).staticLogger("Error 0: "+n.getMessage()+".\nOcurrió en la clase '"+dataWindow4.class.getName()+"', en el método 'botones(miStorePic)'");
                new logger(Level.SEVERE).exceptionLogger(dataWindow4.class.getName(),"botones.miStorePic-0",n.fillInStackTrace());
            }
        });
