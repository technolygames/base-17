...
public class escritorJSON{
    ...
    public void writeDataPartnerJson(int codigoSocio){
        try{
            ...
            rs=ps.executeQuery();
            while(rs.next()){
                new File(System.getProperty("user.dir")+"/src/data/dataBackup/Socios/"+rs.getString("nombre_part")+"-"+rs.getInt("codigo_part")).mkdir();
                jsonw=new JsonWriter(new OutputStreamWriter(new FileOutputStream(System.getProperty("user.dir")+"/src/data/dataBackup/Socios/"+rs.getString("nombre_part")+"-"+rs.getInt("codigo_part")+"/"+rs.getString("nombre_part")+"-"+rs.getInt("codigo_part")+".json"),StandardCharsets.UTF_8));
    ...
    public void writeDataProviderJson(int codigoProveedor){
        try{
            ...
            rs=ps.executeQuery();
            while(rs.next()){
                new File(System.getProperty("user.dir")+"/src/data/dataBackup/Proveedores/"+rs.getString("nombre_prov")+"-"+rs.getInt("codigo_prov")).mkdir();
                jsonw=new JsonWriter(new OutputStreamWriter(new FileOutputStream(System.getProperty("user.dir")+"/src/data/dataBackup/Proveedores/"+rs.getString("nombre_prov")+"-"+rs.getInt("codigo_prov")+"/"+rs.getString("nombre_prov")+"-"+rs.getInt("codigo_prov")+".json"),StandardCharsets.UTF_8));
